...
function runAction() {
  ...
  const useYarn = existsSync(join(pkgRoot, 'yarn.lock'))
  const usePnpm = existsSync(join(pkgRoot, 'pnpm-lock.yaml'))
  log(
    `Will run ${
      useNpm ? 'NPM' : useYarn ? 'Yarn' : 'PNPM'
    } commands in directory "${pkgRoot}"`,
  )
